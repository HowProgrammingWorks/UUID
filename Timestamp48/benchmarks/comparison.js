...
function runComparativeBenchmarks() {
  ...
  const timestamp48Result = benchmark(
  ...
  const batchResult = benchmark(
    ...
    iterations / 100,
  ); // Fewer iterations since each generates 1000 IDs
...
function displayResults(results) {
  ...
  for (const result of results) {
    ...
    console.log(
      `| ${name} | ${opsPerSec} | ${latency} | ${memory} | ${duration} |`,
    );
  }
  ...
  if (results.length > 1) {
    ...
    for (let i = 1; i < results.length; i++) {
      ...
      console.log(
        `   ${fastest.name} is ${speedup.toFixed(
          1,
        )}x faster than ${current.name}`,
      );
    }
  }
  ...
  console.log(
    `   Memory usage: ${mostMemoryEfficient.memoryPerOp.toFixed(
      1,
    )} bytes per operation`,
  );
}
...
function runPerformanceTests() {
  ...
  console.log(
    `Collision handling rate: ${Math.round(
      (10000 / collisionTime) * 1000,
  ...
  console.log(
    `Decode performance: ${decodeResult.opsPerSec.toLocaleString()} ops/sec`,
  );
  console.log(`Average decode latency: ${decodeResult.avgLatencyNs}ns`);

  // Test validation performance
  console.log('\\nTesting validation performance...');
  const validationResult = benchmark(
    'Timestamp48.isValid()',
    () => {
      const randomId = testIds[Math.floor(Math.random() * testIds.length)];
  ...
  console.log(
    `Validation performance: ${validationResult.opsPerSec.toLocaleString()}`,
    'ops/sec',
  );
...
function testMemoryUsage() {
  ...
  console.log(
    `  Range: ${(minGrowth / 1024).toFixed(1)}KB - ${(maxGrowth / 1024).toFixed(
      1,
    )}KB`,
  );
  ...
  console.log(
    `  Leak detected: ${Math.abs(growthTrend) > 25 ? '❌ Possible' : '✅ None'}`,
  );
}
