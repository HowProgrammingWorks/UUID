...
describe('UUIDv7 Standard Compliance', () => {
  describe('48-bit Timestamp Format', () => {
    test('uses 48-bit Unix millisecond timestamp', () => {
      ...
      assert.ok(Math.abs(decoded - now) < 1000, 'Should be within 1 second of current time');
    ...
    test('timestamp range covers UUIDv7 specification', () => {
      ...
      console.log(`Valid range: ${minDate.toISOString()} to ${maxDate.toISOString()}`);
      ...
      // Verify range
      assert.ok(maxDate.getFullYear() >= 2248, 'Should support dates beyond 2248');
    });
    ...
    test('maintains monotonic ordering property', () => {
      ...
      for (let i = 1; i < timestamps.length; i++) {
        assert.ok(timestamps[i] >= timestamps[i - 1], 
          `Timestamp at index ${i} (${timestamps[i]}) < previous (${timestamps[i - 1]})`);
      }
      ...
      assert.deepStrictEqual(sortedIds, sortedByTimestamp, 
  ...
  describe('Sub-millisecond Precision and Collision Handling', () => {
    test('handles multiple IDs within same millisecond', () => {
      ...
      for (const id of ids) {
        ...
        assert.ok(diff <= 1, `Decoded timestamp ${decoded} too far from fixed ${fixedTimestamp}`);
      }
    });
    
    test('counter mechanism provides deterministic sub-millisecond ordering', () => {
      const baseTimestamp = Date.now();
      const idsWithCounters = [];
      
      // Generate IDs with explicit counters
      for (let counter = 0; counter < 10; counter++) {
        const id = Timestamp48._encodeTimestampWithCounter(baseTimestamp, counter);
        const decoded = Timestamp48.decode(id);
      ...
      for (let i = 1; i < idsWithCounters.length; i++) {
        ...
        assert.ok(current.id >= previous.id, 
      ...
      for (const item of idsWithCounters) {
        ...
        assert.ok(diff <= 1, 'Counter should not significantly change timestamp');
      }
    });
    ...
    test('handles counter overflow correctly', () => {
      ...
      const maxCounterId = Timestamp48._encodeTimestampWithCounter(fixedTimestamp, 0xFFF);
      assert.ok(Timestamp48.isValid(maxCounterId), 'Max counter ID should be valid');
      
      // Verify counter stays within 12-bit range
      for (let counter = 0; counter <= 0xFFF; counter += 100) {
        const id = Timestamp48._encodeTimestampWithCounter(fixedTimestamp, counter);
        assert.ok(Timestamp48.isValid(id), `Counter ${counter} should produce valid ID`);
      }
    });
    ...
    test('collision handling preserves chronological order', () => {
      ...
      for (let i = 1; i < timestamps.length; i++) {
        assert.ok(timestamps[i] >= timestamps[i - 1], 
          `Chronological order broken at index ${i}: ${timestamps[i]} < ${timestamps[i - 1]}`);
      }
      
      // Verify lexicographic order matches timestamp order
      const lexicographicSorted = [...ids].sort();
      assert.deepStrictEqual(ids, lexicographicSorted, 
  ...
  describe('Encoding Format Compliance', () => {
    ...
    test('encoding is deterministic', () => {
      ...
      assert.strictEqual(encoded1, encoded2, 'Same timestamp should produce same encoding');
    });
    ...
    test('different timestamps produce different encodings', () => {
      ...
      assert.notStrictEqual(encoded1, encoded2, 'Different timestamps should produce different encodings');
    });
    ...
    test('preserves all 48 bits of timestamp information', () => {
      ...
      for (const timestamp of testCases) {
        ...
        assert.strictEqual(decoded, timestamp, 
          `Failed to preserve timestamp: 0x${timestamp.toString(16)} -> 0x${decoded.toString(16)}`);
      }
    });
  });
  ...
  describe('Base64URL RFC 4648 Section 5 Compliance', () => {
    test('uses correct Base64URL character set', () => {
      ...
      assert.strictEqual(Timestamp48._encodeTable, expectedChars, 'Character set must match RFC 4648 Section 5');
    });
    ...
    test('produces URL-safe output without padding', () => {
      for (let i = 0; i < 1000; i++) {
        ...
        assert.match(id, /^[A-Za-z0-9_-]+$/, 'Should only contain Base64URL characters');
    ...
    test('decode table correctly maps all Base64URL characters', () => {
      ...
      for (let i = 0; i < chars.length; i++) {
        ...
        assert.strictEqual(decoded, i, 
          `Character '${char}' (code ${charCode}) should map to ${i}, got ${decoded}`);
      }
      ...
      for (const char of invalidChars) {
        const charCode = char.charCodeAt(0);
        const decoded = Timestamp48._decodeTable[charCode];
        assert.strictEqual(decoded, 255, `Invalid character '${char}' should map to 255`);
      }
    });
  ...
  describe('Standard Validation', () => {
    test('validates against UUIDv7 timestamp portion requirements', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        assert.ok(timestamp <= Math.pow(2, 48) - 1, 'Timestamp must fit in 48 bits');
        
        // Should be reasonably close to current time (within 1 second)
        assert.ok(Math.abs(timestamp - now) < 1000, 
          `Timestamp ${timestamp} too far from current time ${now}`);
      }
    });
    
    test('complies with Base64URL without padding requirement', () => {
      // 6 bytes = 48 bits should encode to exactly 8 Base64URL characters without padding
      // 48 bits / 6 bits per character = 8 characters exactly
      
      for (let i = 0; i < 100; i++) {
        const id = Timestamp48.generate();
        
        assert.strictEqual(id.length, 8, 'Must be exactly 8 characters (no padding needed)');
