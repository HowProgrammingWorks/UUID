...
describe('Timestamp48 Performance', () => {
  describe('Single Generation Performance', () => {
    test('generates >1M IDs per second (conservative test)', () => {
      ...
      console.log(
        `Performance: ${Math.round(idsPerSecond).toLocaleString()} IDs/second`,
      );

      // Conservative assertion - should easily exceed 1M/sec
      assert.ok(
        idsPerSecond > 1_000_000,
        `Performance target not met: ${Math.round(
          idsPerSecond,
        )} < 1,000,000 IDs/sec`,
      );
    });
    ...
    test('sub-millisecond latency per generation', () => {
      ...
      console.log(
        `Average latency: ${avgLatencyNs.toFixed(0)}ns (${(
          avgLatencyNs / 1000
        ).toFixed(2)}μs)`,
      );
      console.log(
        `Min latency: ${minLatencyNs}ns, Max latency: ${maxLatencyNs}ns`,
      );

      // Most generations should be sub-millisecond (1,000,000 ns)
      const subMillisecondCount = measurements.filter(
        (m) => m < 1_000_000,
      ).length;
      ...
      assert.ok(
        ...
        `${percentage.toFixed(
          1,
        )}% of generations were sub-millisecond (target: >90%)`,
      );
    });
    ...
    test('consistent performance over time', () => {
      ...
      const variance =
        results.reduce(
          (acc, val) => acc + Math.pow(val - avgPerformance, 2),
          0,
        ) / results.length;
      const stdDev = Math.sqrt(variance);

      console.log(
        `Avg: ${Math.round(avgPerformance).toLocaleString()} IDs/sec`,
      );
      console.log(
        `Range: ${Math.round(minPerformance).toLocaleString()} - ${Math.round(
          maxPerformance,
        ).toLocaleString()}`,
      );
      console.log(`Std Dev: ${Math.round(stdDev).toLocaleString()}`);

      // Performance should be relatively stable (CV < 20%)
      const coefficientOfVariation = (stdDev / avgPerformance) * 100;
      assert.ok(
        coefficientOfVariation < 20,
        `Performance too variable: CV=${coefficientOfVariation.toFixed(
          1,
        )}% (target: <20%)`,
      );
    });
  ...
  describe('Batch Generation Performance', () => {
    test('batch generation is faster than individual calls', () => {
      ...
      assert.ok(
        speedupRatio > 2,
        `Batch generation not fast enough: ${speedupRatio.toFixed(
          1,
        )}x speedup (target: >2x)`,
      );
    });
    ...
    test('large batch generation performance', () => {
      ...
      for (const size of sizes) {
        ...
        console.log(
          `Batch ${size.toLocaleString()}: ${durationMs.toFixed(
            2,
          )}ms, ${Math.round(idsPerSecond).toLocaleString()} IDs/sec`,
        );

        assert.strictEqual(batch.length, size);
        assert.ok(
          idsPerSecond > 500_000,
          `Batch ${size} too slow: ${Math.round(idsPerSecond)} IDs/sec`,
        );
      }
    });
    ...
    test('batch generation scales linearly', () => {
      ...
      const maxDeviation = Math.max(
        ...timings.map((t) => Math.abs(t - avgTimePerItem)),
      );
      const deviationPercentage = (maxDeviation / avgTimePerItem) * 100;

      console.log(
        `Average time per item: ${(avgTimePerItem * 1_000_000).toFixed(0)}ns`,
      );
      console.log(`Max deviation: ${deviationPercentage.toFixed(1)}%`);

      assert.ok(
        deviationPercentage < 50,
        `Scaling not linear: ${deviationPercentage.toFixed(
          1,
        )}% deviation (target: <50%)`,
      );
    });
  ...
  describe('Memory Performance', () => {
    test('minimal memory allocation per generation', () => {
      ...
      assert.ok(
        memoryPerGeneration < 100,
        `Too much memory per generation: ${memoryPerGeneration.toFixed(
          2,
        )} bytes (target: <100)`,
      );
    });
    ...
    test('no memory leaks during extended operation', () => {
      ...
      assert.ok(
        growthPercentage < 50,
        `Potential memory leak: ${growthPercentage.toFixed(
          1,
        )}% growth (target: <50%)`,
      );
    });
  ...
  describe('Performance Metrics', () => {
    test('metrics tracking works correctly', () => {
      ...
      console.log(
        `Metrics - Generations: ${metrics.totalGenerations.toLocaleString()}`,
      );
      console.log(
        `Metrics Per: ${metrics.generationsPerSecond.toLocaleString()} IDs/sec`,
      );
    ...
    test('batch generation updates metrics correctly', () => {
      ...
      console.log(
        `Batch metrics Generated: ${metrics.totalGenerations.toLocaleString()}`,
      );
    });
  });
  ...
  describe('Stress Testing', () => {
    test('handles rapid concurrent-like generation', () => {
      ...
      console.log(
        `Stress test: ${iterations.toLocaleString()} unique IDs in ${durationMs.toFixed(
          2,
        )}ms`,
      );
      console.log(
    ...
    test('performance under memory pressure', () => {
      ...
      console.log(
        `Under memory pressure: ${Math.round(idsPerSecond).toLocaleString()} ` +
          'IDs/sec',
      );

      // Should still maintain reasonable performance
      assert.ok(
        idsPerSecond > 100_000,
        `Performance degraded too much under memory pressure: ${Math.round(
          idsPerSecond,
        )} IDs/sec`,
      );
